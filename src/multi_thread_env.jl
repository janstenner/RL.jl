...
function reward(env::MultiThreadEnv)
...
function legal_action_space_mask(env::MultiThreadEnv)
    N = ndims(env.states)
    @sync for i in 1:length(env)
...
action_space(env::MultiThreadEnv) = env.action_space
state_space(env::MultiThreadEnv) = env.state_space
legal_action_space(env::MultiThreadEnv) = Space(legal_action_space.(env.envs))
# current_player(env::MultiThreadEnv) = current_player.(env.envs)
...
mutable struct EnrichedAction
    action
    action_log_prob
end

function EnrichedAction(action; action_log_prob = nothing)
    EnrichedAction(action, action_log_prob)
end
