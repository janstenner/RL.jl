    ...
    Agent(
        policy = PPOPolicy(
            approximator = isnothing(approximator) ? ActorCritic(
                ...
                optimizer_actor = OptimiserChain(ClipNorm(clip_grad), Adam(learning_rate, betas)),
                optimizer_critic = OptimiserChain(ClipNorm(clip_grad), Adam(learning_rate, betas)),
            ) : approximator,
            γ = y,
            λ = p,
            clip_range = 0.2f0,
