...
mutable struct PPOPolicy2{A<:ActorCritic,D,R} <: AbstractPolicy
    approximator::A
    γ::Float32
    λ::Float32
    clip_range::Float32
...
function prob(
    p::PPOPolicy2{<:ActorCritic{<:GaussianNetwork},Normal},
    state::AbstractArray,
    mask,
)
...
function prob(p::PPOPolicy2{<:ActorCritic,Categorical}, state::AbstractArray, mask)
